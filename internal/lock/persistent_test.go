...
// TestPersistentManager_OversizedEventRejectedByLedger verifies that oversized
// events are rejected at the ledger read level, preventing DoS attacks.
// Note: The ledger package enforces size limits before the lock manager sees events.
func TestPersistentManager_OversizedEventRejectedByLedger(t *testing.T) {
	_, dir := createTestLedger(t)

	// Create an event that exceeds ledger.MaxEventSize (1MB)
	// by using an extremely long field
	largeOwner := make([]byte, ledger.MaxEventSize+1)
	...
	// Re-open ledger and try to create manager - should fail because ledger rejects oversized events
	l2, err := ledger.NewLedger(dir)
	if err != nil {
		t.Fatalf("NewLedger() unexpected error: %v", err)
	}
	...
	_, err = lock.NewPersistentManager(l2)
	if err == nil {
		t.Error("NewPersistentManager() with oversized event expected error, got nil")
	}

	// Error should mention size (from ledger level)
	errMsg := err.Error()
...
// Note: TestPersistentManager_OversizedNonLockEventIgnored was removed because
// the ledger package now enforces size limits for ALL events at the read level.
// This is the correct security behavior - oversized events should never be processed
// regardless of type, preventing DoS attacks via maliciously large event files.
