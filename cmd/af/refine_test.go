...
func TestRefineCmd_InvalidInferenceType(t *testing.T) {
	...
	_, err := executeCommand(cmd, "refine", "1",
		"--owner", "test-agent",
		"Some statement",
		// Justification is a free-text label since bb90541 (any non-blank
		// string is accepted); blank is the remaining invalid value.
		"--justification", "   ",
		"--dir", tmpDir,
	)
...
func TestRefineCmd_Help(t *testing.T) {
	...
	// Statements are positional arguments (the --statement flag was removed).
	expectations := []string{
		"refine",
		"--owner",
		"<statement>",
...
func TestRefineCmd_DefCitationValid(t *testing.T) {
	...
	output, err := executeCommand(cmd, "refine", "1",
		"--owner", "test-agent",
		"By def:group, we have a binary operation",
		"--dir", tmpDir,
	)
...
func TestRefineCmd_DefCitationNotFound(t *testing.T) {
	...
	_, err := executeCommand(cmd, "refine", "1",
		"--owner", "test-agent",
		"By def:nonexistent-definition, we have...",
		"--dir", tmpDir,
	)
...
func TestRefineCmd_DefCitationMultipleValid(t *testing.T) {
	...
	output, err := executeCommand(cmd, "refine", "1",
		"--owner", "test-agent",
		"Using def:group and def:homomorphism, we can construct...",
		"--dir", tmpDir,
	)
...
func TestRefineCmd_DefCitationOneInvalid(t *testing.T) {
	...
	_, err = executeCommand(cmd, "refine", "1",
		"--owner", "test-agent",
		"Using def:group and def:ring, we show...",
		"--dir", tmpDir,
	)
...
func TestRefineCmd_DefCitationNoCitations(t *testing.T) {
	...
	output, err := executeCommand(cmd, "refine", "1",
		"--owner", "test-agent",
		"This statement has no definition citations",
		"--dir", tmpDir,
	)
...
func TestRefineCmd_DefCitationHyphenatedName(t *testing.T) {
	...
	output, err := executeCommand(cmd, "refine", "1",
		"--owner", "test-agent",
		"By def:Stirling-second-kind, the number of partitions...",
		"--dir", tmpDir,
	)
...
func TestRefineCmd_WithDependsFlag_MultipleDeps(t *testing.T) {
	...
	output, err := executeCommand(cmd3, "refine", "1",
		"--owner", "test-agent",
		"Combining steps 1.1 and 1.2, we get...",
		"--depends", "1.1,1.2",
		"--dir", tmpDir,
	)
...
func TestRefineCmd_WithDependsFlag_EmptyString(t *testing.T) {
	...
	output, err := executeCommand(cmd, "refine", "1",
		"--owner", "test-agent",
		"Statement without dependencies",
		"--depends", "",
		"--dir", tmpDir,
	)
...
func TestRefineCommand_NextStepsShowsBreadthFirst(t *testing.T) {
	...
	// Output should show the sibling option (now the refine-sibling command,
	// formerly refine --sibling) marked as breadth
	if !strings.Contains(output, "af refine-sibling 1.1") {
		t.Errorf("expected Next steps to show 'af refine-sibling 1.1', got: %q", output)
	}
	if !strings.Contains(output, "(breadth)") {
		t.Errorf("expected Next steps to mark the sibling option as breadth, got: %q", output)
	}
	// Sibling option should come before the depth (add child) option
	siblingIdx := strings.Index(output, "af refine-sibling")
	depthIdx := strings.Index(output, "(depth)")
...
// ===========================================================================
// Issue 4 (vibefeld-cunz): adding siblings. This was the refine --sibling (-b)
// flag; 56e65eb replaced it with the refine-sibling command (vibefeld-yo5e),
// so these tests exercise that command.
// ===========================================================================

// newRefineWithSiblingTestCmd creates a test command hierarchy with both the
// refine and refine-sibling commands.
func newRefineWithSiblingTestCmd() *cobra.Command {
	cmd := newTestRootCmd()
	cmd.AddCommand(newRefineCmd())
	cmd.AddCommand(newRefineSiblingCmd())
	AddFuzzyMatching(cmd)
	return cmd
}

func TestRefineCommand_Sibling_CreatesAtSameLevel(t *testing.T) {
	tmpDir, cleanup := setupRefineTest(t)
	defer cleanup()

	// Create 1.1 first
	cmd := newRefineWithSiblingTestCmd()
	_, err := executeCommand(cmd, "refine", "1",
		"--owner", "test-agent",
		"First child",
		"--dir", tmpDir,
	...
	// Now use refine-sibling on 1.1 to create 1.2 (sibling of 1.1)
	cmd2 := newRefineWithSiblingTestCmd()
	output, err := executeCommand(cmd2, "refine-sibling", "1.1",
		"--owner", "test-agent",
		"Sibling of 1.1",
		"--dir", tmpDir,
	)
	...
	if err != nil {
		t.Fatalf("expected no error with refine-sibling, got: %v", err)
	}
	...
	siblingID, _ := service.ParseNodeID("1.2")
...
func TestRefineCommand_Sibling_ErrorOnRoot(t *testing.T) {
	tmpDir, cleanup := setupRefineTest(t)
	defer cleanup()

	// Try to add a sibling to the root node
	cmd := newRefineWithSiblingTestCmd()
	_, err := executeCommand(cmd, "refine-sibling", "1",
		"--owner", "test-agent",
		"Cannot be sibling of root",
		"--dir", tmpDir,
	)

	if err == nil {
		t.Fatal("expected error when adding a sibling to root, got nil")
	}
...
func TestRefineCommand_Sibling_ShortFlags(t *testing.T) {
	tmpDir, cleanup := setupRefineTest(t)
	defer cleanup()
	...
	// Short flags on refine-sibling
	cmd2 := newRefineWithSiblingTestCmd()
	output, err := executeCommand(cmd2, "refine-sibling", "1.1",
		"-o", "test-agent",
		"Sibling via short flags",
		"-d", tmpDir,
	)
	...
	// Should create 1.2
	if !strings.Contains(output, "1.2") {
		t.Errorf("expected sibling at 1.2 with short flags, got: %q", output)
	}
}

func TestRefineCommand_Sibling_ShowsInHelp(t *testing.T) {
	cmd := newRefineWithSiblingTestCmd()
	output, err := executeCommand(cmd, "refine", "--help")
	...
	// refine's help should point to refine-sibling for adding siblings
	if !strings.Contains(output, "refine-sibling") {
		t.Errorf("expected refine help to mention refine-sibling, got: %q", output)
	}
	if strings.Contains(output, "--sibling") {
		t.Errorf("refine help should not advertise the removed --sibling flag, got: %q", output)
	}
}
