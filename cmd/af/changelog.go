...
var changelog = []release{
	{
		...
		Items: []string{
			...
			"`support_current` (D4) shares the amended result-use relation, so it now sees `local_assume` children like any other child: a pending hypothesis, or a pending step under one, makes its parent `TARGET_PENDING`, and a refuted `local_assume` child is `TARGET_REFUTED`.",
			"Taint has no affected set any more. Because reference and validation dependencies carry taint, a change can reach ancestors, descendants and every transitive reverse dependent, so each recomputation rederives every node and emits a `TaintRecomputed` event for the nodes whose taint actually changed; `taint.PropagateTaint` is now a thin wrapper over `taint.RecomputeAll` (its `root` argument is only a nil guard and the caller's event scope). Replay keeps its single authoritative final derivation (moved to the service load path) so stale historical audit events cannot override derived taint.",
			"`af taint-trace` walks the support relation (child, reference dependency, validation dependency), names the edge kind, words each line by the component the source contributes, and reports the source's recorded verdict sequence or latest statement/dependency amendment: `  1.1 — tainted via child 1.1 (admitted, taint self_admitted, verdict seq 6)` for an admitted child, `  1.1 — unresolved via child 1.1 (needs_refinement, taint unresolved)` for a reopened one. A legacy result-use cycle, whose members are all validated, is named as the cycle rather than printing an empty source list: `  1.1 — unresolved via cycle 1.1 -> 1.2 -> 1.1`. The JSON output adds `support_sources`, each carrying `edge`, `contributes`, `path`, `state`, `taint`, the sequences, and `cycle` for a cycle source.",
			"Verification: an independent recursive specification (`internal/taint/spec_test.go`) written from the rule list, and a seeded differential fuzz (`internal/taint/support_fold_fuzz_test.go`, 3000 random 5-40 node graphs with mixed edges, local_assume scopes, missing targets, legacy cycles, holes in the ID space and random command outcomes) asserting production == spec, idempotence of `RecomputeAll`, a rederivation from stale stored taint == spec, no clean node has a support path through a non-validated result, and sibling separation. A second differential (`internal/taint/ledger_fuzz_test.go`) drives random command sequences -- accept, admit, refute, archive, request-refinement, amend, amend-deps including `--reopen`, unvalidate, and stale `taint_recomputed` audit events -- into a real ledger and compares replay + `RecomputeAll` against the same spec. `scripts/corpus-taint-diff.sh` diffs `af export --graph json` between two binaries over the 211-workspace corpus; only `taint_state` and `validation.taint_counts` are allowlisted to change, and the AISM corpus diff is 0/211 workspaces (the corpus has no reference or validation edge into a non-clean node).",
		},
	},
	{
		Version: "0.1.10",
